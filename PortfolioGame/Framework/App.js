...
export default class App
{
    ...
    Update()
    {
        ...
        if (this.m_deltaSecondsSum >= 1.0)
        {
            ...
            g_debugRenderSystem.AddScreenText("FPS: " + (1.0 / averageDeltaSeconds).toFixed(0), new Vec2(g_aspect * SCREEN_SIZE_Y - 8.0, SCREEN_SIZE_Y - 8.0), 8.0, new Vec2(1.0, 0.5), 1.0, Rgba8.WHITE, Rgba8.WHITE);
            this.m_numFrames = 0;
            this.m_deltaSecondsSum = 0.0;
        }
    ...
    Render()
    {
        ...
        // Background box for FPS text
        const fpsBackgroundBoxVerts = [];
        VertexUtils.AddPCUVertsForAABB2(fpsBackgroundBoxVerts, new AABB2(new Vec2(g_aspect * SCREEN_SIZE_Y - 56.0, SCREEN_SIZE_Y - 16.0), new Vec2(g_aspect * SCREEN_SIZE_Y, SCREEN_SIZE_Y)), Rgba8.BLACK);
        g_renderer.BindShader(null);
        g_renderer.SetBlendMode(BlendMode.ALPHA);
        g_renderer.SetDepthMode(DepthMode.DISABLED);
        g_renderer.SetCullMode(CullMode.BACK);
        g_renderer.BindTexture(null);
        g_renderer.DrawVertexArray(fpsBackgroundBoxVerts);
